<?php
    ...
    if ( !empty($_POST)) 
    {
        ...
        if ($valid) 
        {
               ...
               $sql = "SELECT 1 FROM TB_USUARIO WHERE login='$login' LIMIT 1";
        ...
        if ($valid && $loginValid) 
        {
               $pdo = Database::connect();        
               $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
               $sql = "INSERT INTO TB_USUARIO (login,senha,nome,endereco,telefone,email) values(?, ?, ?, ?, ?, ?)";
               $q = $pdo->prepare($sql);
               $q->execute(array($login,$senha,$nome,$endereco,$telefone,$email));
               Database::disconnect();
...
<?php include("common/header.php"); ?>
    ...
    <div class="container">
                ...
                <div class="span10 offset1">
                    ...
                    <div class="row">
                        <h3>Cadastro de usuário comum</h3>
                    </div>
             
                    <form class="form-horizontal" action="createComum.php" method="post">
